...
def brutal_truth_html(row):
    metrics = [
        ("RELIABILITY", row['Rel_Raw'], row['Reliability_Adj'], "#3a8fb5", "#5fc5e8"),
        ("REPUTABILITY", row['Rep_Raw'], row['Reputability_Adj'], "#9b5fcf", "#c97ef0"),
    ]
    html = ""
    for label, raw, adj, c_raw, c_adj in metrics:
        delta = adj - raw
        delta_html = f'<span class="bt-delta {"delta-neg" if delta < 0 else "delta-pos"}">{delta:+.1f}</span>'
        html += (
            f'<div class="bt-row">'
            f'<div class="bt-label-line">'
            f'<div class="bt-label">{label} {delta_html}</div>'
            f'<div class="bt-values">Raw {raw:.1f} | Adjusted {adj:.1f}</div>'
            f'</div>'
            f'</div>'
        )
    return html
...
if not uploaded_file:
    ...
    with st.sidebar:
        ...
        user_choice = st.selectbox(
            "Select User Report",
            results['UserID'].tolist(),
        )
        st.markdown(f"""
        <div style='font-family: DM Mono, monospace; font-size: 0.6rem; color: #4a5568;
                    letter-spacing: 0.1em; text-transform: uppercase; margin-top: 1rem;'>
            Cohort Size
        </div>
        <div style='font-family: Playfair Display, serif; font-size: 2rem; font-weight: 900; color: #e8e4dc;'>
            {len(results)}
        </div>
        <div style='font-size: 0.75rem; color: #7a8399;'>schools in dataset</div>
        """, unsafe_allow_html=True)
        ...
        <div style='font-family: Playfair Display, serif; font-size: 1.6rem; font-weight: 900; color: #e8e4dc;'>{avg_gi:.1f}</div>
        </div>
        <div style='margin-top: 1rem;'>
        <div style='font-family: DM Mono, monospace; font-size: 0.6rem; color: #4a5568;
                    letter-spacing: 0.1em; text-transform: uppercase;'>Current Rank</div>
        <div style='font-family: Playfair Display, serif; font-size: 1.6rem; font-weight: 900; color: #e8c84a;'>#{rank} / {len(results)}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ════════════════════════════════════
    #  SECTION 1 — Stage Profile + Radar
    # ════════════════════════════════════
    st.markdown(section_header("01", "Growth Stage Profile"), unsafe_allow_html=True)
    ...
    # ════════════════════════════════════
    #  SECTION 2 — R-Score Gauges
    # ════════════════════════════════════
    st.markdown(section_header("02", "R-Cube Maturity Gauges"), unsafe_allow_html=True)
 
    g1, g2, g3 = st.columns(3)
    with g1:
        st.plotly_chart(gauge_chart(row['Relevance'],       "RELEVANCE",       "#e8c84a"), use_container_width=True)
    with g2:
        st.plotly_chart(gauge_chart(row['Reliability_Adj'], "RELIABILITY (ADJ)", "#4ab8e8"), use_container_width=True)
    with g3:
    ...
    # ════════════════════════════════════
    #  SECTION 3 — Brutal Truth
    # ════════════════════════════════════
    st.markdown(section_header("03", "The Brutal Truth — Maturity Adjustment Impact"), unsafe_allow_html=True)
    st.markdown("""
    <div style='font-size: 0.82rem; color: #7a8399; margin-bottom: 1.2rem;'>
        These bars show how weak Relevance foundations drag down higher-level R-scores.
        A negative delta indicates the school's raw operational or reputational scores
        are partially negated by foundational weakness.
    </div>
    """, unsafe_allow_html=True)
 
    bt1, bt2 = st.columns(2)
    with bt1:
        st.markdown(brutal_truth_html(row), unsafe_allow_html=True)
    with bt2:
        # Comparison bar chart
        bt_df = pd.DataFrame({
            'Metric':  ['Reliability', 'Reputability'],
            'Raw':     [row['Rel_Raw'],  row['Rep_Raw']],
            'Adjusted':[row['Reliability_Adj'], row['Reputability_Adj']],
        })
        fig_bt = go.Figure()
        fig_bt.add_trace(go.Bar(name='Raw',      x=bt_df['Metric'], y=bt_df['Raw'],      marker_color='#2a3347'))
        fig_bt.add_trace(go.Bar(name='Adjusted', x=bt_df['Metric'], y=bt_df['Adjusted'], marker_color=['#4ab8e8','#b87ae0']))
    ...
    # ════════════════════════════════════
    #  SECTION 4 — Question-Level Grid
    # ════════════════════════════════════
    st.markdown(section_header("04", "Question-Level Score Breakdown"), unsafe_allow_html=True)
    st.markdown("""
    <div style='font-size: 0.82rem; color: #7a8399; margin-bottom: 0.5rem;'>
        Each cell shows the 0–100 converted score for that question.
        Colour maps to band: <span style='color:#ef4444;'>■ Fragile</span>
        <span style='color:#f59e0b;'>■ Emerging</span>
        <span style='color:#3b82f6;'>■ Developing</span>
        <span style='color:#10b981;'>■ Strong</span>
        <span style='color:#a78bfa;'>■ Benchmark</span>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(question_grid_html(row), unsafe_allow_html=True)
 
    st.markdown("<hr>", unsafe_allow_html=True)
 
    # ════════════════════════════════════
    #  SECTION 5 — Strategic Action Path
    # ════════════════════════════════════
    st.markdown(section_header("05", "Strategic Action Path"), unsafe_allow_html=True)
 
    alerts = get_action_alerts(row)
    for cls, title, body in alerts:
        st.markdown(alert_html(cls, title, body), unsafe_allow_html=True)
    ...
    # ════════════════════════════════════
    st.markdown(section_header("06", "Growth Index Distribution — All Users"), unsafe_allow_html=True)
    st.plotly_chart(distribution_chart(results), use_container_width=True)

    # Cohort summary table
    st.markdown("<br>", unsafe_allow_html=True)
    summary = results[['UserID', 'Growth_Index', 'Relevance', 'Reliability_Adj', 'Reputability_Adj']].copy()
    summary.columns = ['User', 'Growth Index', 'Relevance', 'Reliability (Adj)', 'Reputability (Adj)']
    summary = summary.sort_values('Growth Index', ascending=False).reset_index(drop=True)
    summary.index += 1
    for col in ['Growth Index', 'Relevance', 'Reliability (Adj)', 'Reputability (Adj)']:
