...
if not uploaded_file:
    ...
    <div style='margin-bottom: 1.25rem;'>
        ...
        <div style='font-family: Playfair Display, serif; font-size: 2.4rem; font-weight: 900;
                    color: #1a1c20; line-height: 1.1; margin-bottom: 0.75rem;'>
            {user_choice}
        </div>
    ...
    #  KPI ROW
    # ════════════════════════════════════
    # KPI row wrapped in flex container for equal height
    st.markdown(f"""
    <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; margin-bottom:1.5rem;'>
        {kpi_card("Growth Index",       row['Growth_Index'],     "growth",       "growth")}
        {kpi_card("Relevance",          row['Relevance'],        "relevance",    "relevance")}
        {kpi_card("Reliability (Adj)",  row['Reliability_Adj'],  "reliability",  "reliability")}
        {kpi_card("Reputability (Adj)", row['Reputability_Adj'], "reputability", "reputability")}
    </div>
    """, unsafe_allow_html=True)
